...
| 8 | Reporte mensual: un solo archivo HTML, abre sin internet, gráficas correctas | ✅ Verificado | `tests/test_reports.py` (sin `src/href` remotos, 2 SVG); generado en vivo en Modo B y validado estructuralmente (viewBox, coordenadas) |
...
| 11 | `docker compose up -d` levanta; datos y reportes sobreviven a `down && up`; `/healthz` 200; dashboard exige usuario/contraseña | ✅ Verificado en vivo | Compose ejecutado: healthy, healthz 200 sin auth, 401/401/200 según credenciales, 6 conexiones y 1 reporte conservados tras `down && up`, usuario no root (1000) |
| 12 | Credenciales no legibles en `monitor.db`; en Modo B sin `MONITOR_SECRET_KEY` no se descifran | ✅ Verificado en vivo | `grep` del secreto en el .db del volumen → no aparece (token `fernet:…`); modo docker exige la clave (`tests/test_secrets.py`) |
| 13 | Sin tráfico distinto de los chequeos configurados | ✅ Por construcción + verificado parcial | Cero dependencias de red en runtime (Chart.js/fuentes locales, sin telemetría); tráfico identificable verificado en vivo (`pg_stat_activity` → `StabilityMonitor/0.1.0`, 0 sesiones remanentes). Inspección con Wireshark pendiente como parte del smoke en destino |
