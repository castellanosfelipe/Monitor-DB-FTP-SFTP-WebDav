...
next to the executable when frozen with PyInstaller (Windows), the repository
root during development, or wherever ``MONITOR_DATA_DIR`` points.
"""
...
def base_dir() -> Path:
    ...
    if env:
        return Path(env)
    if getattr(sys, "frozen", False):  # PyInstaller onedir bundle (Modo A)
